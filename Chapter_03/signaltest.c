...
main(int argc, char *argv[], char *envp[])
{
    ...
    /* SIGHUPのシグナルハンドラを指定 */
#ifdef USE_SIGNAL
    /* signalシステムコールによるシグナルハンドラ指定
       補足：Linux環境では初回のシグナルちゃんと受け取るが、そこでexecve()
       してしまうため、シグナルハンド処理が完了とならず、同じシグナルが届かない。
     */
...
#else
    /* sigactionシステムコールによるシグナルハンドラ指定  */
    (void) sigaction(SIGHUP, (struct sigaction *) NULL, &sa);
    sa.sa_handler = sig_hangup_handler;
    sa.sa_flags = SA_NODEFER; //シグナルの内部の時でもシグナルを妨害しないようにする
